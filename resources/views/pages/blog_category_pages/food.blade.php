            ...
            <div class="row">
                <div class="col-lg-8 mt--10 ">
                    ...
                    <div class="card-columns">
                        <!--Post-Start-->
                        @foreach($blogs_food as $key => $food)

                        <div class="card">
                            <div class="post-card">
                            <div class="post-card-image">
                                <a href="{{route('allpost' , [$food->id,$food->category])}}">
                                    <img src="{{url($food->image)}}" alt="">
                                </a>
                            </div>
                            <div class="post-card-content">
                                <a href="{{route('allpost' , [$food->id,$food->category])}}" class="categorie">{{$food->category}}</a>  
                        ...
                        @endforeach
                         ...
                         <div class="pagination mt-30">
                            @if ($blogs_food->lastPage() >1)
                            <ul class="list-inline">
                                <li class="{{$blogs_food->currentPage() == 1 ? 'disabled' : ''}}"><a class="" href="{{$blogs_food->url(1)}}"><i class="arrow_carrot-2left"></i></a>
                                    @for($i=1; $i<=$blogs_food->lastPage();$i++)
                                    <li class="siam {{$blogs_food->currentPage() == $i ? 'current' : ''}}">
                                      <a class='page-numbers' href='{{$blogs_food->url($i)}}'>{{$i}}</a></li>
                                    @endfor
                                    <li class="{{$blogs_food->currentPage() == $blogs_food->lastPage() ? 'disabled' : '' }}"><a class="disabled" href="{{$blogs_food->url($blogs_food->currentPage() +1 )}}"><i class="arrow_carrot-2right"></i></a></li>
